...
PLUGIN_PATH = File.expand_path(File.join(RAILS_ROOT,"vendor/plugins/formal"))
...
namespace :formal do
  ...
  task :install do
    ...
    css = Dir.glob(File.join(PLUGIN_PATH,"stylesheets","*.css"))
    
    FileUtils.cp_r(css, public_css_path)
    
    puts "All Formal files installed!"
    puts "------------------------------------------"
    puts "just <%= stylesheet_link_tag 'formal' %> in the head of"
